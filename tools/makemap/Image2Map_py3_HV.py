...
class TileMap:
    ...
    def __init__(self, file, tileX, tileY):
        ...
        self.TileImage2 = self.getTileImage2()

        # Create an image file from our map of unique tiles.
        self.TilemapImage = self.getTilemapImage()
    ...
    def getTileImage(self):
        ...
        print("tilecount",tilecount,"total tile pixel count", tilecount*self.TileX*self.TileY)
        print("world map pixel count",self.MapImageWidth * self.MapImageHeight,"tile pixels per total pixels", (tilecount*self.TileX*self.TileY) / (self.MapImageWidth * self.MapImageHeight))
        return TileImage

    def getTileImage2(self):
        """ This function takes the hash of unique tiles self.FullTileMap and
            creates a tileset image from it.
        """
        TileImage = Image.new("RGB", (self.TileX *len(self.TileList), self.TileY * 1) )

        for i in range(len(self.TileList)):
            box = ( self.TileX * i , 0, self.TileX * (i + 1), self.TileY ) 
            TileImage.paste(self.TileList[i].convert("RGB"), box)

        print("len(self.TileList)",len(self.TileList))
        return TileImage

    def getTilemapImage(self):
        """ This function takes the hash of unique tiles self.FullTileMap and
            creates a tileset image from it.
        """
        ...
        TilemapImage = Image.new("L", (self.Width, self.Height) )
        print("tilemap size", self.Width, self.Height)        

        tilecount = 0;
        for i in range(self.Height-1):
            for j in range(self.Width-1):
                #print("i & j", i, j, "=>", H[i][j]-1)
                #TilemapImage[i][j] = H[i][j]
                TilemapImage.putpixel((j,i), H[i][j]-1)
    ...
    def addEdge(self, s, t, dirr):
        ...
        if self.G.has_edge(s, t):
            values = [ value for value in self.G.adj[s][t] if (dirr * value) > 0 ]
        else:
            values = []
    ...
    def growTileMap(self, TileMap, G, posX, posY, curV):
        ...
        dir.sort(key=cmp_to_key(lambda L1, L2: len(L1[0]) - len(L2[0])))
        ...
        while dir != []:
            ...
            if Edges[direction] != []:
                ...
                # "belong together".
                E.sort(key=cmp_to_key(lambda e, f: abs(e[1]) - abs(f[1])), reverse = True)
...
if sys.argv[1] == "--help":
    ...
    for file in sys.argv[3:]:
        ...
        tilefile2 = os.path.splitext(file)[0] + "-Tileset2" + ".png"
        print ("Saving the tileset image into the file: " + tilefile2)
        map.TileImage2.save( tilefile2, "PNG" )

        #print ("Pretty-printing the tileset:" + "\n")
        # map.printHash(map.FullTileMap)
        
        tilemapfile = os.path.splitext(file)[0] + "-Tilemap" + ".png"
        print ("Saving the tilemap image into the file: " + tilemapfile)
        map.TilemapImage.save( tilemapfile, "PNG" )
